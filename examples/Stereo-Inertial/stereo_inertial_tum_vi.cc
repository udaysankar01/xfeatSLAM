                ...
                vector<string> &vstrImageLeft, vector<string> &vstrImageRight, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            {
                std::string timestamp_str = s.substr(0, pos);
                std::string item = s.substr(pos + 1);
                
                vstrImageLeft.push_back(strPathLeft + "/" + item);
                vstrImageRight.push_back(strPathRight + "/" + item);
